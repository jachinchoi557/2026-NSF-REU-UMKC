"""
Post-hoc video pass — every 10th episode with recovery v2 enabled.

Mirrors record_sampled_episodes_recovery.py exactly, adding RecoveryState
from recovery_logic_v2. A fresh RecoveryState is instantiated per episode.

Videos saved to tairo_results/videos/with_recovery_v2/<condition>/episode_<N>.mp4.

RNG matching guarantee
----------------------
Uses the same seed logic as run_failure_analysis_recovery_v2.py: one env per
condition, 100 sequential env.reset() calls, throwaway loops for non-sampled
episodes. Recovery state is also tracked during throwaway episodes so that
sampled episode N is in the identical system state as in the CSV.
Attack closure state is reset between every episode via on_reset.
"""
...
VIDEO_ROOT      = os.path.join("tairo_results", "videos", "with_recovery_v2")
FPS             = 30

# ---------------------------------------------------------------------------
# Attack factories — identical to run_failure_analysis_recovery_v2.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single-episode runner with recovery v2; returns frames list
# ---------------------------------------------------------------------------
